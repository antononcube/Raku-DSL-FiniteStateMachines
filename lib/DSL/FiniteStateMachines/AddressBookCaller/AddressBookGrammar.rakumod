role DSL::FiniteStateMachines::AddressBookCaller::AddressBookGrammar {
    ...
    rule call-filter {
        <.call-preamble>? [<call-list-pos-spec>    | <call-filter-company>    | <call-filter-occupation> [<call-from-company> ]?    | <call-filter-age>    | <contact-name> ]
    }
    rule call-from-company {
        [<.from-preposition>    | <.of-preposition> ] <.the-determiner>? <.company-noun>? <company-name>
    }
    rule call-filter-company { [<.third-person-singular> <.is-verb> ] <call-from-company>    | [<.the-determiner> 'one' ]? <call-from-company>  }
    rule call-filter-occupation {
        [<.call-preamble>?    | [<.third-person-singular> <.is-verb> ]? [<.a-determiner>    | <.the-determiner> ]] <contact-occupation>
    }
    rule call-filter-age {
        <.the-determiner>? ['younger'    | 'older' ] 'one'?
    }
    rule call-list-pos-spec {
        <.the-determiner>? [<call-list-num-pos>    | 'last'    | 'former'    | 'latter' ] ['one'?    | ['one'? <.in-preposition> <.the-determiner> <.list-noun> ]? ]
    }
    rule call-list-num-pos { <position-query-link>  }
    # from DSL::Shared::Roles::English::ListManagementCommand
    rule call-meeting-host {
        <.call-verb> <.the-determiner> 'host' <.of-preposition> <.the-determiner> 'meeting' <time-spec>
    }
    rule call-meeting-number {
        <.call-verb> <call-meeting-number>
    }
